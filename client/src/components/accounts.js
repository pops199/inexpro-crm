...
const Accounts = (() => {
    ...
    try {
      ...
      if (headerActions) {
        headerActions.innerHTML = `
          ...
          <a class="btn btn-secondary" href="/api/accounts/${id}/report.pdf" target="_blank" rel="noopener">Print</a>
  ...
  async function _openAttachmentLibrary(opts, alreadyPicked, onApply) {
    ...
    //
    // The ROA ("Advice") template is hidden — ROAs are signed via the
    // per-advice-record entries under Engagements (only the records on
    // THIS account show).
  ...
  async function _sendMail() {
    ...
    // Library picks now cover docs, claim-form templates, the Policy
    ...
    (modal?._libraryDocs || []).forEach(d => {
      switch (d.type) {
        ...
        case 'roa':
          // Per-ROA pick → signature_request for that specific record.
          signablePicks.push({
