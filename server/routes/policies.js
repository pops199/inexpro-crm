...
// POST /:id/git-confirmation/sign-request ────────────────────────────────
// Same payload as the download endpoint, but instead of returning the PDF
// directly we save the form data, create a pending signature_request
// linked to this policy + its contact/account, and return the public
// signing URL the broker can share with the client.
router.post('/:id/git-confirmation/sign-request', async (req, res, next) => {
  try {
    const db = getDb();
    const policy = db.prepare('SELECT * FROM policies WHERE id = ?').get(req.params.id);
    if (!policy) return res.status(404).json({ error: 'Policy not found' });
    ...
    // Resolve insured name + email from the policy chain so the
    // signature_request carries enough context for the signing page.
    let recipientName = null;
    let recipientEmail = null;
    if (policy.contact_id) {
      const c = db.prepare('SELECT first_name, last_name, email FROM contacts WHERE id = ?').get(policy.contact_id);
      if (c) {
        recipientName = `${c.first_name || ''} ${c.last_name || ''}`.trim() || null;
        recipientEmail = c.email || null;
      }
    } else if (policy.account_id) {
      const a = db.prepare('SELECT account_name FROM accounts WHERE id = ?').get(policy.account_id);
      if (a) recipientName = a.account_name || null;
    }
    if (!recipientName && formData.insured_name) recipientName = String(formData.insured_name);
    ...
    const result = db.prepare(`
      ...
      VALUES (?, 'git_confirmation', 'pending', ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      token,
      policy.contact_id || null,
      policy.account_id || null,
      policy.id,
      req.session.userId,
      expiresAt,
      recipientName,
    ...
    res.locals.logAudit({
      ...
      description: `GIT Confirmation signature request created for policy ${policy.policy_number || policy.id}`,
    });
    ...
    console.error('POST /policies/:id/git-confirmation/sign-request error:', err);
    return next(err);
  }
});
